...
export function WeatherModal({ trip, isOpen, onClose }) {
      ...
      <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl border border-slate-100 overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-sky-600 via-sky-700 to-blue-800 p-6 text-white relative">
          ...
          <div className="flex items-center gap-2 text-sky-200 text-xs font-semibold uppercase tracking-wider mb-1">
            <Compass className="w-4 h-4" /> Live Destination Weather
          </div>
          ...
          <div className="mt-6 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-sky-200 text-sm font-medium">{weather.tempF} · {weather.condition}</div>
              </div>
            </div>
            <div className="text-right text-xs space-y-1 text-sky-100">
              <div className="flex items-center gap-1.5 justify-end">
                <Wind className="w-3.5 h-3.5 text-sky-300" /> Wind: {weather.wind}
              </div>
              <div className="flex items-center gap-1.5 justify-end">
                <Droplets className="w-3.5 h-3.5 text-sky-300" /> Humidity: {weather.humidity}
              </div>
              <div className="flex items-center gap-1.5 justify-end">
                <Thermometer className="w-3.5 h-3.5 text-amber-300" /> UV Index: {weather.uvIndex} (High)
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 bg-slate-50/50">
          ...
          <div className="mt-5 p-3.5 bg-sky-50 rounded-xl border border-sky-100 flex items-start gap-3">
            ...
            <div className="text-xs text-sky-900 leading-relaxed">
              <strong>Travel Tip:</strong> Mediterranean & Aegean islands experience breezy evenings (Meltemi winds). Pack a light linen cardigan or windbreaker for sunset cliffside dinners.
            </div>
          </div>
        </div>
